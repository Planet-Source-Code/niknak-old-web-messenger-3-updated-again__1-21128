...
Begin VB.Form frm_messenger 
   ...
   Begin MSComctlLib.StatusBar sta_info 
      ...
      EndProperty
   End
   Begin AgentObjectsCtl.Agent msa_buddy 
      Left            =   1980
      Top             =   0
   End
...
Private Sub setup_message(pos As Integer)
    Dim back_colour As String
    back_colour = wm_colvars(colvar_page).variable_colour_html
        Select Case pos
            Case Is = start_message
                Open App.Path + messages_path + messages_file For Output As #1
                    ...
                    Print #1, "</head>"
                    Print #1, "<body bgcolor=""" & back_colour & """>"
                Close #1
            Case Is = end_message
...
Private Sub begin_session()
    ...
    cdlg.Filter = "Html file (*.html)|*.html|"
    '*****************
    toggle_controls
    load_settings
...
Private Sub men_savemess_Click()
    Dim fs, fil1
    Set fs = CreateObject("Scripting.FileSystemObject")
    Set fil1 = fs.GetFile(App.Path + messages_path + messages_file)
...
Private Sub load_buddy()
    buddy = search_buddies(win_tcpip.RemoteHostIP)
    If buddies(buddy).buddy_file <> "" Then
        If buddies(buddy).buddy_enabled = 1 Then
            msa_buddy.Characters.load buddies(buddy).buddy_name, buddies(buddy).buddy_file
            msa_buddy.Characters(buddies(buddy).buddy_name).Show
            use_buddy = True
        End If
    End If
End Sub
...
Private Function search_buddies(search As String) As Integer
    ...
    For c_buddy = 0 To max_buddies
        If buddies(c_buddy).buddy_ip = search Then
...
Private Sub win_tcpip_DataArrival(ByVal bytesTotal As Long)
    ...
    'PRINT MESSAGE
    If strData_passed <> "" Then
        write_message strData_passed, True
        'BUDDY TALK
        If use_buddy = True Then msa_buddy.Characters(buddies(buddy).buddy_name).Speak strData_passed
    End If
End Sub
...
Private Sub write_message(message As String, recieved As Boolean)
    ...
    If recieved = True Then
        text_colour = wm_colvars(colvar_rxmsg).variable_colour_html
        snd_events(event_onrx).start
    Else
        text_colour = wm_colvars(colvar_txmsg).variable_colour_html
...
Private Sub toggle_controls()
    ...
    men_options.Enabled = Not men_options.Enabled
